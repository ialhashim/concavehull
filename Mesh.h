...
namespace{
    static inline Eigen::Vector3d cross(Eigen::Vector3d a, Eigen::Vector3d b){ return a.cross(b); }
    static inline double dot(Eigen::Vector3d a, Eigen::Vector3d b){ return a.dot(b); }
    static inline double norm(Eigen::Vector3d a) { return a.norm(); }
}
...
public: //------------------------------------------------------ property types
    ...
    public:
        ...
        /// access the data stored for vertex \c v
        typename Property<T>::reference operator[](Vertex_iterator v)
        {
            return Property<T>::operator[]((*v).idx());
        }

        /// access the data stored for vertex \c v
        typename Property<T>::const_reference operator[](Vertex_iterator v) const
        {
            return Property<T>::operator[]((*v).idx());
        }
    };
...
public: //----------------------------------------------- add new vertex / face
    ...
    Face add_triangle(int v1, int v2, int v3);
...
Surface_mesh::Face
Surface_mesh:: add_triangle(int v1, int v2, int v3)
{
    return add_triangle(Vertex(v1), Vertex(v2), Vertex(v3));
}
...
{
    const unsigned int n((int)vertices.size());
    assert (n > 2);
