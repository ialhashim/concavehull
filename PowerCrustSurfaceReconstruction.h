// Adapted from https://github.com/timhutton/vtkpowercrust

/*===================================================================================================

PowerCrustSurfaceReconstruction algorithm reconstructs surfaces from unorganized point data.
...
public:
	PowerCrustSurfaceReconstructionImpl(void);
...
PowerCrustSurfaceReconstructionImpl::PowerCrustSurfaceReconstructionImpl(void)
{
}
...
void PowerCrustSurfaceReconstructionImpl::outputPole(simplex* pole, int poleid, double* samp, int* num_poles, double distance)
{
	...
	medial_surface.InsertWeight(weight);
...
void PowerCrustSurfaceReconstructionImpl::construct_face(simplex *s, short k)
{
	...
	//output.InsertNextCell(numedges);
	...
	if (!correct_orientation(plane[0], plane[1], plane[2], inpole, outpole))
		...
		{
			{
				//output.GetPolys()->InsertCellPoint(atoi(indface[i]));
				indices.push_back(atoi(indface[i]));
			}
		}
	else
	{
